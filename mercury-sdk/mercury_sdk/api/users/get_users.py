...
from ...models.users_paginated_response import UsersPaginatedResponse
...
def _get_kwargs(
    *,
    ...
    order: GetUsersOrder | Unset = GetUsersOrder.ASC,
) -> dict[str, Any]:
...
) -> Any | UsersPaginatedResponse | None:
    if response.status_code == 200:
        response_200 = UsersPaginatedResponse.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | UsersPaginatedResponse]:
    return Response(
...
def sync_detailed(
    *,
    ...
    Args:
        ...
        start_after (UUID | Unset): The ID of the user to start the page after (exclusive). When
            provided, results will begin with the user immediately following this ID. Use this for
            standard forward pagination to get the next page of results. Cannot be combined with
            end_before.
        end_before (UUID | Unset): The ID of the user to end the page before (exclusive). When
        ...
        order (GetUsersOrder | Unset): Sort order. Can be 'asc' or 'desc'. Defaults to 'asc'
            Default: GetUsersOrder.ASC.
